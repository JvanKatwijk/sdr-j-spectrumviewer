...
//	and be prepared for future changes in the settings
	...
	connected	= true;
        ...
        float Fpass     = 0.8 * PLUTO_RATE / 2;
        float Fstop     = Fpass * 1.25;
...
void	plutoHandler::setVFOFrequency	(uint64_t newFrequency) {
int	ret;
...
uint64_t	plutoHandler::getVFOFrequency () {
	return this -> lo_hz;
}
//
...
bool	plutoHandler::legalFrequency	(uint64_t f) {
	(void)f;
	return true;
}

uint64_t	plutoHandler::defaultFrequency	() {
	return 94700000;
}
