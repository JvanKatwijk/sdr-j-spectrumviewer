...
uint64_t	sdrplayHandler_v3::defaultFrequency	() {
	return Mhz (220);
}

uint64_t	sdrplayHandler_v3::getVFOFrequency	() {
	return vfoFrequency;
}

void	sdrplayHandler_v3::setVFOFrequency	(uint64_t newFreq) {
vfoRequest r ((int32_t)newFreq);
...
int32_t	amount;
	...
	_I_Buffer. skipDataInBuffer (segmentSize - amount);
	return amount;
}
...
//	set the parameter values
	...
	deviceParams	-> devParams -> fsFreq. fsHz	= (float)SAMPLERATE;
...
//	we signal the caller when we are done
	   switch (server_queue. front () -> cmd) {
	      ...
	      case SETFREQUENCY_REQUEST: {
	         vfoRequest *p =
	                 (vfoRequest *)(server_queue. front ());
	         server_queue. pop ();
	         chParams -> tunerParams. rfFreq. rfHz =
	                                            (float)(p -> frequency);
	         fprintf (stderr, "Frequency wordt gezet op %d\n",
	                                            p -> frequency);
                 err = sdrplay_api_Update (chosenDevice -> dev,
                                           chosenDevice -> tuner,
                                           sdrplay_api_Update_Tuner_Frf,
                                           sdrplay_api_Update_Ext1_None);
                 if (err != sdrplay_api_Success) {
                    fprintf (stderr, "set frequency: error %s\n",
                                      sdrplay_api_GetErrorString (err));
	            p -> result = false;
                 }
	         else
