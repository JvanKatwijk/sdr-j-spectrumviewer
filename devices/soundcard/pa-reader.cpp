 ...
 */
	...
	paReader::paReader 	(int32_t cardRate,
	                         QComboBox *s, bool *success) {
int16_t	i;
...
int32_t	paReader::getSamples	(DSPCOMPLEX *V,
	                         int32_t n, int32_t segment) {
float	*buf = (float *)alloca (2 * n * sizeof (float));
int32_t	i, realAmount;
int32_t	skipAmount	= 2 * segment - 2 * n;

	if (!portAudio)
	   return 0;

	while (_I_Buffer -> GetRingBufferReadAvailable () < 2 * segment) {
	   if (!readerRunning)
	      return 0;
	   Pa_Sleep (1);
	}
...
int16_t		i;
	...
	s -> insertItem (0, "no device", QVariant (0));
	for (i = 0; i < numofDevices; i ++) {
